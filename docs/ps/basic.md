...
11. 复制当前选区：Alt + 移动工具（复制当前选区或者当前图层），水平拖动：按shift、Alt键变换选区

## 套索工具 L
> 套索工具、多边形套索工具、磁性套索工具

1. 套索工具：按住左键，拖动
2. 多边形套索工具：按住左键，拖动（按esc键:删除全部；或者拖错，按delete键，删除最后一个点）
...
2. 修复画笔工具：先按Alt键取干净的位置，直接对污点区域点击左键即可，硬度一般设置最小，一般情况下会保持一点点纹理
3. 修补工具：用鼠标左键按住拖动圈选位置，将鼠标放在选区内，按住鼠标左键拖动到好的区域，最后取消选区
4. 污点修复画笔工具：修复图像的污点—--职阶对污点区域点击左键即可，硬度一般设置最小
5. 红颜工具：用工具在红颜或有彩色的位置框选出一个矩形框，中心位置就被去色
...
2. 完全替换修复：先按Alt键取新位置，直接在旧位置点击左键即可，硬度一般设置最小
3. 图案图章工具：用图案局部填充到图层或选区内，替换原有的色彩或图像（用新图案时，先定义图案（编辑--定义图案），再使用）
...
1. 橡皮带：勾选：显示两点之间的连线；不勾选，不显示两点之间的连线
2. 安装Ctrl键，左键点击空白处，即可切断当前钢笔的连线，从新开始路径（即路径不闭合也可以开始新的路径）
3. 按Ctrl可以选择和修改路径结点（按住Ctrl键后，点击某个结点，然后拖动、调节等），按shift键约束水平、垂直、和45度方向
4. 按Alt键可以角点和平滑点的转换---按Alt键后在平滑点上点击，即可转为角点；按Alt键后在角点上点击+拖移，即可转为平滑
5. 路径转为选取：Ctrl + Enter
